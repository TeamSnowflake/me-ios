...
class MAHelloWorldViewController: UIViewController {
    ...
    @IBAction func sendRequest(_ sender: Any) {
        ...
        Alamofire.request("http://requestbin.fullcontact.com/1jttdvr1", method: .post, parameters: (newMessageJSON as! Parameters), encoding: JSONEncoding.default, headers: nil).responseJSON { (response) in
            print(response)
        }
    }
    ...
    func getMessages(){
        Alamofire.request("https://forus.io/api/v1/messages").responseJSON { response in
            ...
            if let json = response.result.value {
                let messages = try! Messages(object: json as! JSONObject)
                print(messages)
            }
